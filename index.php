...
// Proteksi halaman user: jika user adalah admin, redirect ke admin.php
$user = getUser();
if ($user && isset($user['role']) && $user['role'] === 'admin') {
    header('Location: ' . route('admin/admin.php'));
    exit;
}
        ...
        <div class="sidebar-card">
            ...
            <ul class="category-list">
                ...
                <?php foreach ($categories as $cat): ?>
                    <?php if ($cat !== 'Semua'): ?>
                        <li class="category-item">
                            <a href="<?= route('user/category.php?brand=' . urlencode($cat)) ?>"><?= htmlspecialchars($cat) ?></a>
                        </li>
                    <?php endif; ?>
                <?php endforeach; ?>
            </ul>
        ...
        <div class="product-grid">
            <?php foreach ($products as $product): ?>
                <article class="product-card">
                    <a href="<?= route('user/product.php?id=' . $product['id']) ?>" class="product-thumb">
                        <img src="<?= htmlspecialchars($product['image']) ?>" alt="<?= htmlspecialchars($product['name']) ?>">
                    </a>
                    <div class="product-meta">
                        <span class="tag"><?= htmlspecialchars($product['brand']) ?></span>
                        <h3><a href="<?= route('user/product.php?id=' . $product['id']) ?>"><?= htmlspecialchars($product['name']) ?></a></h3>
                    ...
                    <form class="product-actions" method="post">
                        ...
                        <a href="<?= route('user/product.php?id=' . $product['id']) ?>" class="btn-secondary">Lihat Detail</a>
                    </form>
                </article>
            <?php endforeach; ?>
